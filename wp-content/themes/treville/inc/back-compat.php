 ...
 *
 * Prevents Treville from running on WordPress versions prior to 4.4,
 ...
 * Treville on WordPress versions prior to 4.4.
 *
 * @global string $wp_version WordPress version.
 */
function treville_upgrade_notice() {
	$message = sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'treville' ), 'Treville', '4.4', $GLOBALS['wp_version'] );
...
function treville_customize() {
	wp_die( sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'treville' ), 'Treville', '4.4', $GLOBALS['wp_version'] ), '', array(
		'back_link' => true,
	) );
}
...
function treville_preview() {
	if ( isset( $_GET['preview'] ) ) {
		wp_die( sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'treville' ), 'Treville', '4.4', $GLOBALS['wp_version'] ) );
	}
}
